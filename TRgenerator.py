...
class TR_multiMotif:
    ...
    def generate_seq(self):
        ...
        l = self.length
        sublen = np.random.randint(1, int(l / 4), size = 100)
        sublen[99] = l - sum(sublen[0:99])
        submotif = np.random.choice(self.motif_list, size = 100)
        #print(sublen)
        #print(submotif)
        ...
        for i in range(100):
            ...
            if l <= 0:
                break
            ###print(submotif[i], min(sublen[i],l), self.mutation_rate, seed)
            subTR = TR_singleMotif(submotif[i], min(sublen[i],l), self.mutation_rate, self.seed)
            seq += subTR.sequence
...
if __name__ == "__main__":
    seed = 55889615
    #hsat3 = TR_singleMotif('AATGG', 1000, 0.1, seed)
